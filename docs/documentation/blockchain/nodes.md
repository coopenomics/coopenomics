...
Чтобы их использовать активно, каждый желающий может запустить HISTORY-ноду, включив дополнительный плагин, который сделает исторические данные доступными по протоколу **State History (SHiP)** поверх WebSocket (см. [State History (SHiP)](state-history-ship.md)). Для подписки на этот поток и десериализации в прикладные потоки в экосистеме COOPOS поставляется **[Реактивный чтец SHiP](../reactive-ship-reader.md)**; классические пайплайны также выгружают данные в **MongoDB** или **PostgreSQL**. **State-ноду** при желании **совмещают с API-нодой** на одном хосте или выносят отдельно — см. [state-нода](configuration/state-node.md).
...
На **light-нодах** тот же **Chain API** используется для **локальной** работы приложений кооператива: состояние соответствует цепи **с момента снимка** и далее по новым блокам, без полного архива прошлого. Профиль см. [light-нода](configuration/light-node.md).
