---
content_title: Спецификация кошелька COOPOS
---
...
- 2. В начало добавляется байт 0x80. Он соответствует mainnet Bitcoin; в COOPOS используется тот же байт версии. При кодировании байт версии помогает отличить запись как закрытый ключ. В отличие от Bitcoin, в COOPOS всегда используются сжатые открытые ключи (производные от закрытого), поэтому к закрытому ключу не добавляется суффикс 0x01.
